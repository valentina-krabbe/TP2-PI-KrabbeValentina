...
def segment_image(image_pil, processor, model, color_map):
    ...
    for idx in np.unique(pred_mask):
        ...
        if label in color_map:
            ...
            # Generar color aleatorio consistente para etiquetas no mapeadas
...
def detect_faces_dnn(image_cv_bgr, net):
    ...
    # 1. Crear un blob (preprocesamiento)
...
def detect_orb_keypoints(image_cv_bgr):
    ...
    # nfeatures=1000 es un buen número para análisis urbano
...
def apply_clahe(image_cv_bgr, clip_limit, tile_grid_size):
    ...
    # 1. Convertir a espacio de color LAB (L=Luminosidad)
    lab = cv2.cvtColor(image_cv_bgr, cv2.COLOR_BGR2LAB)
    l, a, b = cv2.split(lab)
    
    # 2. Crear el objeto CLAHE
    clahe = cv2.createCLAHE(clipLimit=clip_limit, tileGridSize=(tile_grid_size, tile_grid_size))
    
    # 3. Aplicar CLAHE al canal de Luminosidad (L)
    cl = clahe.apply(l)
    
    # 4. Combinar los canales de nuevo
    limg = cv2.merge((cl, a, b))
    
    # 5. Convertir de nuevo a BGR
...
def apply_adaptive_threshold(image_cv_bgr, method_option, block_size, C_value):
    ...
    # block_size debe ser impar y mayor que 1
    if block_size % 2 == 0:
        block_size += 1 
...
def apply_morphological_op(image_cv_bgr, op_option, kernel_size):
    ...
    # Se recomienda usar la imagen ya umbralizada para morfología (si es el caso)
